...
+ (PGDatePicker *)show {
    ...
    static PGDatePicker *datePicker = nil;
    if (datePicker) {
        datePicker.dismissView.hidden = false;
        [UIView animateWithDuration:0.3 animations:^{
            datePicker.headerView.frame = CGRectMake(0, kScreenHeight - height - height1, kScreenWidth, height1);
            datePicker.frame = CGRectMake(0, CGRectGetMaxY(datePicker.headerView.frame), kScreenWidth, height);
        }];
    }
    dispatch_once(&onceToken, ^{
        ...
        datePicker = [[PGDatePicker alloc]initWithFrame:frame];
        [window addSubview:datePicker];
        datePicker.headerView = view;
        
        UILabel *label = [[UILabel alloc]init];
        [view addSubview:label];
        label.textColor = [UIColor colorWithHexString:@"#848484"];
        label.font = [UIFont boldSystemFontOfSize:17];
        datePicker.titleLabel = label;
        [datePicker.titleLabel addObserver:datePicker forKeyPath:@"text" options:NSKeyValueObservingOptionNew context:nil];
        ...
        [cancel addTarget:datePicker action:@selector(cancelButtonHandler) forControlEvents:UIControlEventTouchUpInside];
        [view addSubview:cancel];
        datePicker.cancelButton = cancel;
        ...
        [confirm addTarget:datePicker action:@selector(cancelButtonHandler) forControlEvents:UIControlEventTouchUpInside];
        [view addSubview:confirm];
        datePicker.confirmButton = confirm;
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:datePicker action:@selector(cancelButtonHandler)];
        [dismissView addGestureRecognizer:tap];
        [window addSubview:dismissView];
        datePicker.dismissView = dismissView;
    });
